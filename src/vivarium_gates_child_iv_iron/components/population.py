...
class BasePopulationNew:
    """Component for aging simulants."""
    ...
    def setup(self, builder: Builder) -> None:
        ...
        columns = ["age", "sex", "alive", "location", "entrance_time", "exit_time", "birth_weight", "gestational_age"]
    ...
    def on_initialize_simulants(self, pop_data: SimulantData) -> None:
        ...
        born_previous_step = (birth_records['birth_date'] < self.clock()) & (birth_records['birth_date'] > self.clock() - self.step_size())
        newborn_records = birth_records[born_previous_step]

        # not exactly what we want
        new_simulants = pd.DataFrame(
            {'age': 0,
             'sex': newborn_records['sex'],
             'alive': 'alive',
             'location': 'South Asia', # update
             'entrance_time': self.clock,
             'exit_time': pd.NaT,
             'birth_weight': newborn_records['birth_weight'], # risk exposure
             'gestational_age': newborn_records['gestational_age']}# risk exposure
        )
